...
Как выбранный стек должен загружать общедоступные статические и JavaScript-страницы, ждать готовности, однократно прокручивать до Целевой области, объединять совпадения нескольких Целевых селекторов, применять несколько Селекторов исключения, ограничивать время и выдавать однозначные Результаты проверки?
...
1. **Проверить входные данные до запуска браузерной работы.** URL обязан быть абсолютным `http:` или `https:` URL без встроенных имени пользователя и пароля. Монитор обязан содержать хотя бы один Целевой селектор; каждый Целевой селектор и каждый Селектор исключения обязаны разбираться нативным `document.querySelectorAll`. Синтаксически неверный селектор — ошибка конфигурации, а не «ничего не найдено»; точные дубли после удаления внешних пробелов не принимаются.
...
4. **Проверить ответ главного ресурса.** Redirect разрешён; исследуется фактически открытый главный документ и записывается итоговый `page.url()`. `page.goto` не бросает исключение на `404` или `500`, поэтому статус финального ответа проверяется явно: HTTP `400+` даёт `http_error`, а успешный ответ неподдерживаемого типа — `unsupported_content` ([`page.goto`](https://playwright.dev/docs/api/class-page#page-goto)).
5. **Дождаться Целевой области.** Каждый Целевой селектор разрешается отдельно, после чего совпадения объединяются по идентичности DOM-узла и упорядочиваются по положению в документе. Ожидание успешно, когда объединение непусто и хотя бы одна цель отрисована. Если до конца бюджета объединение пусто — `target_not_found`; если цели есть, но ни одна не отрисована, — `target_not_visible`.
6. **Выполнить ровно одну попытку прокрутки.** Якорь — первая отрисованная цель объединения в порядке документа. Она однократно прокручивается к центру viewport с мгновенным поведением. Нельзя вызывать single-element locator без явного якоря: такие операции strict и при нескольких совпадениях бросают исключение ([strictness](https://playwright.dev/docs/locators#strictness), [`scrollIntoViewIfNeeded`](https://playwright.dev/docs/api/class-locator#locator-scroll-into-view-if-needed)). Прокрутка к последнему элементу, обход всех совпадений, пагинация и бесконечная прокрутка в MVP не выполняются.
7. **Дождаться краткой стабильности после прокрутки.** Проверка стабильности начинается не раньше чем через одну секунду после прокрутки. Включённая проекция Целевой области должна не меняться непрерывно `750` мс. Ожидание ограничено пятью секундами. Изменения целевых узлов, их включённых потомков, состава объединения совпадений или границ Исключений сбрасывают тихое окно; мутации целиком внутри уже определённого исключённого поддерева его не сбрасывают. Наблюдение реализуется через `MutationObserver`, а не безусловный `waitForTimeout` ([MutationObserver](https://dom.spec.whatwg.org/#mutation-observers), [почему `waitForTimeout` discouraged](https://playwright.dev/docs/api/class-page#page-wait-for-timeout)). Если тихого окна не возникло, Проверка завершается `content_unstable`, а не сохраняет случайное промежуточное состояние.
8. **Атомарно извлечь результат.** Один синхронный, не содержащий `await`, вызов `page.evaluate` заново разрешает все Целевые селекторы, строит уникальное DOM-упорядоченное объединение целей, применяет Исключения отдельно внутри каждой цели, строит сериализуемое представление структуры и видимого текста и возвращает его в Node.js. Только после успешного возврата прикладной слой может строить Снимок и сохранять Результат проверки. Контекст закрывается в `finally` при любом исходе.
...
- `querySelectorAll` возвращает статический набор совпадений; после объединения результатов всех Целевых селекторов порядок целей — tree order, то есть preorder depth-first, а не порядок полей формы ([`querySelectorAll`](https://dom.spec.whatwg.org/#dom-parentnode-queryselectorall), [tree order](https://dom.spec.whatwg.org/#concept-tree-order)).
...
### Несколько Целевых селекторов, Исключения и исчезновение узлов

Финальный набор целей всегда определяется заново непосредственно на границе извлечения. Каждый Целевой селектор может совпасть с одним или многими элементами. Все совпадения всех Целевых селекторов объединяются по идентичности узла, поэтому один элемент, совпавший с несколькими селекторами, входит в результат один раз. Затем уникальные цели сортируются в tree order всего документа; порядок Целевых селекторов в настройках на результат не влияет. Разные вложенные цели остаются отдельными элементами набора: содержимое вложенной цели может присутствовать и в представлении родительской цели, и в собственной записи.

Предпросмотр при создании или изменении Области наблюдения показывает число совпадений каждого Целевого селектора и объединённый результат; подтверждение блокируется, пока хотя бы один Целевой селектор не нашёл совпадения. Во время последующей Проверки исчезновение совпадений только одного селектора при непустом общем объединении является успешным новым состоянием и обнаруживается как Изменение. Только пустое объединение всех Целевых селекторов даёт `target_not_found`/`target_disappeared`. Это позволяет наблюдать удаление одной выбранной части страницы, не маскируя его ошибкой.

Нормативные примеры порядка:

- если в DOM элементы идут `B`, `C`, `A`, то Целевые селекторы, введённые как `A`, `B`, `C`, дают цели `B`, `C`, `A`;
- если над повторяющимися карточками идёт `B`, а в каждой карточке по порядку встречаются `A`, затем `C`, результатом будет `B`, `A`, `C`, `A`, `C` и так далее по числу карточек;
- если один DOM-элемент одновременно совпал с `A` и `C`, он присутствует в результате один раз.

Каждый Селектор исключения применяется **независимо относительно каждого target root**; порядок Селекторов исключения не влияет на объединение удаляемых поддеревьев:
...
Внутри одного синхронного page callback выполняются: повторный `querySelectorAll` для каждого Целевого селектора, объединение и DOM-сортировка целей, построение границ Исключений, обход каждой цели и создание plain JSON без DOM handles. Между целями нет возврата в Node event loop, поэтому страница не может вставить отдельную browser task между их чтением. Результат содержит эффективный URL, HTTP-статус главного документа, диагностическое число совпадений каждого селектора, общее число уникальных целей, упорядоченные сырые деревья без Исключений, видимый текст и timings; нормализация Снимка и алгоритм Сравнения остаются билету «Определить формат Снимка и алгоритм Сравнения».
