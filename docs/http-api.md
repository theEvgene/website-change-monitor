...
- `GET /api/checks` — общий Журнал всех Проверок, от новых к старым; каждая запись содержит исторический `url` страницы, использованный этой Проверкой;
- `GET /api/labels` — отсортированные Метки, связанные хотя бы с одним существующим Монитором;
...
Ответ содержит Целевые области и отдельные построчные секции `structure` и `text`. Удаления находятся слева, добавления — справа, перемещение представляется как удаление и добавление. Текстовая строка может содержать `beforeLinks` или `afterLinks`: это список безопасных абсолютных HTTP(S) адресов с диапазонами символов, сохранённый вместе со Снимком. Каждый диапазон отображается как кликабельный фрагмент текста и не участвует в определении Изменения. При превышении встроенного бюджета ответ устанавливает `complete: false` и сообщает точное число пропущенных строк вместо ложного результата «изменений нет».
...
Telegram-уведомление об Изменении использует то же Сравнение, что и локальный интерфейс, и добавляет короткие plain-text секции `➕ Добавлено` и `➖ Удалено`. Замена показывается как отдельные удаление и добавление. Подробности ограничены 300 Unicode-символами: при наличии обеих секций каждая сначала получает по 150, а свободный остаток передаётся другой. Сокращённое или неполное Сравнение отмечается строкой `…показана только часть изменений`. При полном структурном Изменении без изменения видимого текста уведомление сообщает об этом отдельно. Имя Монитора и безопасная исходная ссылка сохраняются вне бюджета Подробностей.

Если пару Снимков получить не удалось либо подготовка Подробностей завершилась ошибкой, dispatcher всё равно отправляет прежнюю базовую форму с заголовком, безопасной ссылкой и телом Уведомления. Успешная fallback-доставка отмечается как `delivered`. В NDJSON-журнал best effort записываются только идентификаторы доставки и Проверки, стабильный этап и безопасные сведения о типе ошибки; Снимки, URL, diff, текстовые фрагменты и готовое Telegram-сообщение туда не попадают. Ошибка самого журнала не блокирует отправку.
